...
from temporal_granularity.src.metrics.multi_year_metrics import MultiYearMetrics
from temporal_granularity.src.models.manipulations.approximations import ApproximateData
from temporal_granularity.src.models.manipulations.duration_curves import get_group_ldc
...
if __name__ == '__main__':
    ...
    for method in ['medoids', 'centroids', "wards"]:
    # for method in ["wards"]:
        for i in range(10):
            logger.info("Iteration number {}".format(i))
            for n_clusters in [2, 4, 8, 16, 32, 48, 61]:
    ...
    results_dataframe.to_csv("{}/temporal_granularity/reports/data/long_term_method_comparison/long_term_method_comparison_wards.csv".format(project_dir))
